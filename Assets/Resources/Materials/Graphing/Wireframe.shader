Shader "Wireframe"
{
	...
	SubShader
	{
		...
		Pass
		{
			...
			{
				float4 projectionSpaceVertex : SV_POSITION;
				float4 worldSpacePosition : Color;
...
//				UNITY_INITIALIZE_OUTPUT(v2g, o);
			...
			fixed4 frag (g2f i) : SV_Target
			{
				...
				fixed3 gradient = cyan * sqrt(pow(i.texcoord.x,2)+pow(i.texcoord.y,2)) / sqrt(2) 
				+ yellow * (1-sqrt(pow(i.texcoord.x,2)+pow(i.texcoord.y,2))/sqrt(2)) 
				+ red * sqrt(pow(i.texcoord.x,2)+pow(1-i.texcoord.y,2)) /sqrt(2)
				+ green * (1-sqrt(pow(i.texcoord.x,2)+pow(1-i.texcoord.y,2))/sqrt(2));

				//return fixed4(gradient, 1);
				return fixed4(i.texcoord.y, i.texcoord.x, 1-i.texcoord.y, 1);
			}
			ENDCG
		}
	}
