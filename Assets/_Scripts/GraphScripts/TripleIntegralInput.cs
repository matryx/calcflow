...
internal class TripleIntegralInputResponder : FlexMenu.FlexMenuResponder
{
    internal bool isReady = false;
    internal TextMesh uMinText, uMaxText, vMinText, vMaxText, wMinText, wMaxText,
        currText;
    ...
    internal List<string> wMinExp = new List<string>();
    internal List<string> wMaxExp = new List<string>();

    List<string> currExpression = new List<string>();
    int index = 0;
    int maxDisplayLength = 20;

    internal TripleIntegralInputResponder(TextMesh umin, TextMesh umax, TextMesh vmin, TextMesh vmax, TextMesh wmin, TextMesh wmax)
    {
        uMinText = umin;
        uMaxText = umax;
        vMinText = vmin;
    ...
    public void initialize()
    {
        ...
        wMinExp = new List<string>
        {
            "0"
        };
        wMaxExp = new List<string>
        {
            "1", "0", "-", "x", "-", "y"
        };
        ...
        wMinText.text = displayText(wMinExp, wMinExp.Count, false);
        wMaxText.text = displayText(wMaxExp, wMaxExp.Count, false);
    ...
    public void Flex_ActionStart(string name, FlexActionableComponent sender, GameObject collider)
    {
        switch (sender.name)
        {
            ...
            case "vmax":
                ...
                break;
            case "wmin":
                currExpression = wMinExp;
                currText = wMinText;
                index = wMinExp.Count;
                break;
            case "wmax":
                currExpression = wMaxExp;
                currText = wMaxText;
                index = wMaxExp.Count;
                break;
        }
...
public class TripleIntegralInput : MonoBehaviour
{
    ...
    public TextMesh uMinInput, uMaxInput, vMinInput, vMaxInput, wMinInput, wMaxInput;
    TripleIntegralInputResponder responder;

    //public CustomVectorField vectorField;
    public TripleIntegralRegion integral;
    ...
    void Update()
    {
        if (responder.isReady)
        {
            ...
            integral.expressionXMin = compileTokens(responder.uMinExp);
            integral.expressionXMax = compileTokens(responder.uMaxExp);
            integral.expressionYMin = compileTokens(responder.vMinExp);
            integral.expressionYMax = compileTokens(responder.vMaxExp);
            integral.expressionZMin = compileTokens(responder.wMinExp);
            integral.expressionZMax = compileTokens(responder.wMaxExp);
