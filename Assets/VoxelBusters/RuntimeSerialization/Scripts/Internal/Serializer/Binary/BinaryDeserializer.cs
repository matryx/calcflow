...
namespace VoxelBusters.RuntimeSerialization.Internal
{
    internal class BinaryDeserializer
    {
        ...
        protected ObjectDebugReader DebugReader;
        ...
        internal BinaryDeserializer()
        {
            ...
            DebugReader = new ObjectDebugReader();
        ...
        internal T Deserialize<T>(byte[] _serializationData, T _targetObject)
        {
            try
            {
                ...
                object _deserializedObj;
                try
                {
                    _deserializedObj = _objectReader.ReadObjectValue(BinaryStreamReader, out _deserializedType, _targetObject);
                }
                catch (Exception e)
                {
                    ...
                    BinaryStreamReader = new RSBinaryReader(512);
                    BinaryStreamReader.LoadBuffer(_serializationData);
                    _serializationFormatVersion = GetVersionInfo();
                    DebugReader.Reset();

                    DebugReader.ReadObjectValueEntry(BinaryStreamReader, out _deserializedType, _targetObject);
                    throw e;
                }
