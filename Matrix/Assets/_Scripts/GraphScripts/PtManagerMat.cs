...
public class PtManagerMat : MonoBehaviour
{
    [HideInInspector]
    ...
    [SerializeField]
    PresentPlaneMatrix presentPlaneMatrix;
    ...
    public GeneratePlanePtsMat generatePlanePtsMat;
    ...
    void Update()
    {
        ...
        if (inputReceived && !eqnInput)
        {
            ...
            if (isValid)
            {
                if (presentPlaneMatrix.CalculatePlane())
                {
                    presentPlaneMatrix.ApplyGraphAdjustment();
                    presentPlaneMatrix.GetLocalPoint();
                    presentPlaneMatrix.GetPlaneDirection();
                }
                else
                {
                    presentPlaneMatrix.ApplyGraphAdjustment();
        ...
        if (inputReceived && eqnInput)
        {
            ...
            if (isValid)
            {
                if (eqnSet.eqnCoefs["a"].Value == 0 && eqnSet.eqnCoefs["b"].Value == 0 && eqnSet.eqnCoefs["c"].Value == 0)
                {
                    ...
                    presentPlaneMatrix.forwardPlane.GetComponent<MeshRenderer>().enabled = false;
                    presentPlaneMatrix.backwardPlane.GetComponent<MeshRenderer>().enabled = false;
                }
                else
                {
                    generatePlanePtsMat.eqnToPoints();
                }
            }
        }
    }
    ...
    public void updatePoint(string ptName, Vector3 newLoc, bool fixedPlane)
    {
        ...
        else
        {
            ...
            if (isValid)
            {
                if (presentPlaneMatrix.CalculatePlane())
                {
                    presentPlaneMatrix.ApplyUnroundCenter(ptName, newLoc);
                    presentPlaneMatrix.GetPlaneDirection();
                }
            }
        }
    ...
    public void eqnUpdatePoint(Vector3 pt1NewLoc, Vector3 pt2NewLoc, Vector3 pt3NewLoc)
    {
        ...
        presentPlaneMatrix.GetLocalPoint();
        presentPlaneMatrix.GetPlaneDirection();
        presentPlaneMatrix.forwardPlane.GetComponent<MeshRenderer>().enabled = true;
        presentPlaneMatrix.backwardPlane.GetComponent<MeshRenderer>().enabled = true;
    }
